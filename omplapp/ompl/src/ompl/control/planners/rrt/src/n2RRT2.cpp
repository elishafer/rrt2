...
ompl::control::n2RRT2::n2RRT2(const SpaceInformationPtr &si) : base::Planner(si, "nRRT2-int")
{
    specs_.approximateSolutions = true;
    siC_ = si.get();

    Planner::declareParam<double>("goal_bias", this, &n2RRT2::setGoalBias, &n2RRT2::getGoalBias, "0.:.05:1.");
    Planner::declareParam<double>("wx", this, &n2RRT2::setWx, &n2RRT2::getWx, "0.:1.0:1.");
    Planner::declareParam<double>("wc", this, &n2RRT2::setWc, &n2RRT2::getWc, "0.:0.08:1.");
}

ompl::control::n2RRT2::~n2RRT2()
{
    freeMemory();
}

void ompl::control::n2RRT2::setup()
{
    base::Planner::setup();
    if (!nn_)
...
void ompl::control::n2RRT2::clear()
{
    Planner::clear();
    sampler_.reset();
...
ompl::base::PlannerStatus ompl::control::n2RRT2::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    ...
    while (ptc == false)
    {
        ...
        if (!isInit)
        {
            ...
            conVals[0] = 0.; // set the change of speed to 0
        }
        if (cd >= siC_->getMinControlDuration())
        {
            ...
            if (solv && !isInit)
            {
                OMPL_INFORM("Updating cmax after solving.");
                OMPL_INFORM("Init cmax = %f", motion->cost);
                isInit = true;
                cmax_ = motion->cost;
            }
...
void ompl::control::n2RRT2::getPlannerData(base::PlannerData &data) const
{
